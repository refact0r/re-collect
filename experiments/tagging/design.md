...
The feature has shipped through Tier 1 and answered several of the open
questions below; the rest of this document is preserved as the original
design rationale. Where they differ, `architecture.md` (as-built) wins:

- **Shipped:** four output fields (`styles`, `subject`, `aiTags`,
  `paletteHex`/`paletteNames`) on items; searchText integration (Tier 1);
  `retagItem` mutation; deterministic hex palette + named-color words.
- **`kind` was cut** — never made it into the prompt or schema.
- **The LLM palette-description string was cut** — color words
  (`paletteNames`) are derived deterministically from `paletteHex` via Lab
  nearest-neighbor (`colorNames.ts`), not generated by the model.
- **`subject` was kept** — it feeds searchText, resolving that open question.
- **Not built yet:** Tier 2/3 surfaces (`/tags` page, chips, "more like
  this"), embeddings/`semanticText`, regenerate-tags UI.
